...
public class FadeManager : MonoBehaviour
{
			...
			if (!this.isFading) {
				...
				foreach (string sceneName in scenes) {
					if (GUI.Button (new Rect (20, 55 + i * 25, 100, 20), "Load Level")) {
						LoadScene (sceneName, 1.0f, 1.0f, 1.0f);
					}
	...
	public void LoadScene (string scene, float begin, float middle, float end)
	{
		StartCoroutine (TransScene (scene, begin, middle, end));
	}
	...
	/// <param name='scene'>シーン名</param>
	/// <param name='interval'>暗転にかかる時間(秒)</param>
	private IEnumerator TransScene (string scene, float begin, float middle, float end)
	{
		//だんだん暗く .
		this.isFading = true;
		float time = 0;
		while (time <= begin) {
			this.fadeAlpha = Mathf.Lerp (0f, 1f, time / begin);      
			time += Time.deltaTime;
			yield return 0;
		}
		...
		time = 0;
		while (time <= end) {
			this.fadeAlpha = Mathf.Lerp (1f, 0f, time / end);
			time += Time.deltaTime;
			yield return 0;
		}
